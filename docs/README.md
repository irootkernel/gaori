...
The standalone v0.1 baseline, recorded hardening, schema-v2 canonical tags, explicit parser and ad-hoc timeout selection, release-readiness follow-up, identity migration, coding-agent guidance, operator-directed standalone cleanup, portable project-config policy, and CLI usability improvements are implemented. The current surfaces include configured and tagged ad-hoc execution, position-independent global options, read-only config/rule preflight, parser-selectable raw-log summarization, excerpt lookup, eleven built-in parsers, rule lifecycle commands including verified summary-failure proposals, bounded/redacted derived evidence, collision-free standalone artifacts, fixed run-scoped artifacts, deterministic status JSON, and fail-closed cleanup by explicit age or `--all`. Parent projects may commit `.gaori/tester.yaml` and reviewed `.gaori/tester/rules/*.yaml`; all other `.gaori/` content remains local-only.
