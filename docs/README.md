...
The standalone v0.1 baseline, the recorded hardening requirements, schema-v2 canonical tags, explicit parser selection, release-readiness follow-up, identity migration, and coding-agent adoption guidance are implemented. The current v0.1.9 surfaces include configured and tagged ad-hoc execution, parser-selectable raw-log summarization, excerpt lookup, eleven built-in parsers, rule lifecycle commands, bounded/redacted derived evidence, collision-free standalone artifacts, fixed run-scoped artifacts, and deterministic status JSON. All `.gaori/` content is local-only state.
