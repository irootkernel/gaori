...
| `GAORI-REQ-RQCLI-008` | `TestHelpSurfacesExitSuccessfully`; `TestHelpDoesNotConsumeChildArguments`; `TestHelpAfterPositionalOptionNameIsRecognized`; `TestUnknownHelpTopicFailsClosed`; `TestRootUsageListsEveryDispatchedCommand`; `TestRulesUsageListsEveryDispatchedSubcommand`; `TestBinaryHelpHierarchy` |
...
| `GAORI-REQ-RQEXT-002` | `TestValidateAcceptsImplementedParsers`; `TestProcessVitestFixture`; `TestProcessPytestFixture`; `TestProcessGoTestFixture`; `TestProcessPlaywrightFixture`; `TestProcessExtendedParserFixtures`; `TestParserIndicatesFailure`; `TestGoTestFailureVariants`; `TestFrameworkParsersFromCapturedStream`; `TestFrameworkParsersFromRawLogFile`; `TestConfiguredRunsUseSpecializedParsers`; `TestBinaryExtractionContracts` |
...
| `GAORI-REQ-RQRUL-010` | `TestLoadProposalsReportsEveryCandidateIncludingRepeatedRuleIDs`; `TestLoadProposalsIgnoresNonYAMLAndMissingDirectory`; `TestLoadProposalByNameResolvesAndFailsClosed`; `TestLoadProposalsDoNotParticipateInExtractionSelection`; `TestRulesProposalsListAndShowCompleteTheReviewLoop`; `TestRulesProposalsReportsEmptyWithoutCandidates`; `TestRulesProposalsRejectsUnsupportedInput` |
...
| `GAORI-REQ-RQSEC-003` | `TestExecuteReportsRawLogWriteFailure`; `TestExecuteTimeoutReportsRawLogWriteFailure`; `TestExecuteInterruptedReportsRawLogWriteFailure`; `TestRawLogWriteFailureDoesNotPublishDerivedArtifacts`; `TestProcessRulesRejectsInvalidRegex`; `TestDiscoverRejectsSpecialRuleFile`; `TestDiscoverProposalsRejectsSpecialProposalFile`; `TestBinaryRejectsUnknownConfigFields`; `TestBinaryRejectsOversizedRuleContext` |
...
| `GAORI-REQ-RQSEC-006` | `TestApplyCountedReportsSequentialMatchesAndMatchesApplyOutput`; `TestApplyCountedReportsZeroForPatternConsumedByEarlierPattern`; `TestApplyCountedPreservesCaptureGroupReplacement`; `TestConfigCheckSampleReportsPerPatternCountsWithoutEchoingContent`; `TestConfigCheckSampleUsesOrderedRedactionPassCounts`; `TestConfigCheckSampleReportsNoConfiguredPatterns`; `TestConfigCheckSampleFailsClosedOnUnusableSample`; `TestConfigCheckSampleRejectsSpecialSampleFile`; `TestConfigCheckWithoutSampleOmitsRedactionSampleField`; `TestBinaryConfigCheckSampleReportsCountsWithoutLeakingSampleContent`; `TestDocumentedCLIWorkflowAgainstFreshFixture` |
...
| `GAORI-REQ-RQMCP-004` | `TestMCPManagerWaitDoesNotCancelAndExplicitCancelFinishes`; `TestMCPCancelAcceptedMeansFirstRequestWasRecorded`; `TestMCPManagerCloseCancelsActiveRun`; `TestWaitMCPInvocationsUsesOneDrainContext`; `TestStartGateSerializesCancellationAfterProcessStart`; `TestBinaryMCPEOFCancelsActiveProcessGroup`; `TestBinaryMCPSignalsShutDownServerAndProcessGroup`; `TestBinaryMCPLifecycleAndBoundedEvidence` |
...
| `GAORI-REQ-RQMCP-007` | `TestMCPServerAdvertisesExpectedTools`; `TestMCPListRunsMatchesCLIRunsListSelectors`; `TestMCPListRunsBoundsAndReportsTruncation`; `TestMCPListRunsBoundsOversizedListingsToTheResponseBudget`; `TestMCPListRunsRejectsInvalidSelectorsWithoutReflectingRequestData`; `TestMCPListRunsFailsClosedOnUnsafeEvidence`; `TestMCPListRunsRejectsCallerSelectedOutputDirectory`; `TestListStandaloneReportsCompletedRunsNewestFirst`; `TestListStandaloneSkipsIncompleteAndUnrecognizedRuns`; `TestBinaryMCPListsCompletedStandaloneEvidence`; `TestMCPDocumentationAndSkillContract` |
| `GAORI-REQ-RQMCP-008` | `TestMCPServerAdvertisesExpectedTools`; `TestMCPAwaitRunReturnsTerminalSnapshotAndSupportsFinishedFastPath`; `TestMCPManagerAwaitReturnsFinishedSnapshot`; `TestMCPAwaitCancellationDoesNotCancelRunAndLaterAwaitSucceeds`; `TestMCPAwaitSupportsConcurrentWaiters`; `TestMCPInvocationLookupErrorsAreBoundedAndNonReflective`; `TestBinaryMCPLifecycleAndBoundedEvidence`; `TestBinaryMCPExitsCleanlyOnEOF`; `TestBinaryMCPSignalsShutDownServerAndProcessGroup`; `TestAwaitRunDocumentationContract` |
