...
- Lost run evidence paths: use `gaori --json runs list` and match on command ID, tags, status, and exit code. A listing failure with exit `3` means the evidence itself is unsafe or malformed; stop and report it instead of reading around it.
