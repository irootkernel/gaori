...
For MCP, an unchanged `wait_run` response is normal and the command remains active. An unknown invocation ID or disconnected server cannot be repaired or recovered: reconcile the external command and exact final artifact paths before considering a retry. After `cancel_run`, continue waiting for `finished` when the session remains available, then verify the returned status and artifacts.
