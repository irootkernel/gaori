...
4. Read the most authoritative machine-readable surface available:
   ...
   - for parser labels: `gaori --json parsers list` is the read-only index of supported labels, and `gaori --json parsers detect <raw-log>` reports what each label would find in an existing log without loading config or creating anything; it reports candidates and never selects a label for you;
