...
func TestDocumentedCLIWorkflowAgainstFreshFixture(t *testing.T) {
	...
	if !strings.Contains(summarized, "Summary: .gaori/runs/scoped/summarize-example/artifacts/test/unit.summary.md") {
	...
	markdownPath := filepath.Join(repo, ".gaori", "runs", "scoped", "summarize-example", "artifacts", "test", "unit.summary.md")
	...
	storedRule, err := os.ReadFile(filepath.Join(repo, ".gaori", "tester", "rules", "generic-v1.yaml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, expectedExit := range []int{0, 1, 0} {
		...
		if i == 2 && (!strings.Contains(output, "# Gaori Summary: demo") || !strings.Contains(output, "token=<redacted>")) {
			t.Fatalf("README summary inspection did not show the documented redacted evidence: %s", output)
		}
	}
	entries, err := os.ReadDir(filepath.Join(readmeRepo, ".gaori", "runs", "standalone"))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 1 {
	...
	readmeRunDir := filepath.Join(readmeRepo, ".gaori", "runs", "standalone", entries[0].Name())
	...
	if err := os.MkdirAll(filepath.Join(integrationRepo, ".gaori"), 0o755); err != nil {
		t.Fatal(err)
	}
	integrationConfig := markdownCodeBlockAfter(t, integrationGuide, "Create `.gaori/tester.yaml`", "yaml")
	if err := os.WriteFile(filepath.Join(integrationRepo, ".gaori", "tester.yaml"), []byte(integrationConfig), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(integrationRepo, "go.mod"), []byte("module example.com/gaori-docs\n\ngo 1.26\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func assertDocumentedArtifacts(t *testing.T, repo, runID string) {
	t.Helper()
	base := filepath.Join(repo, ".gaori", "runs", "scoped", runID, "artifacts", "test")
