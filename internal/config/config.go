...
func Load(repoRoot, override string, allowMissing bool) (model.Config, string, error) {
	...
	if err != nil {
		...
		return model.Config{}, path, model.NewGaoriError(model.ExitCodeConfigError, "read config", err)
	}

	var cfg model.Config
	if err := safety.DecodeYAMLStrict(data, &cfg); err != nil {
		return model.Config{}, path, model.NewGaoriError(model.ExitCodeConfigError, "parse config", err)
	}
	if cfg.Commands == nil {
...
func Validate(cfg model.Config) error {
	if cfg.Version != 2 {
		return model.NewGaoriError(model.ExitCodeConfigError, "validate config", fmt.Errorf("unsupported config version %d", cfg.Version))
	}
	for id, cmd := range cfg.Commands {
		if err := safety.ValidateArtifactIdentifier("command id", id); err != nil {
			return model.NewGaoriError(model.ExitCodeConfigError, "validate config", err)
		}
		if len(cmd.Command) == 0 {
			return model.NewGaoriError(model.ExitCodeConfigError, "validate config", fmt.Errorf("command %q must define argv command", id))
		}
		for _, part := range cmd.Command {
			if strings.TrimSpace(part) == "" {
				return model.NewGaoriError(model.ExitCodeConfigError, "validate config", fmt.Errorf("command %q contains empty argv item", id))
			}
		}
		if err := tagset.Validate(cmd.Tags); err != nil {
			return model.NewGaoriError(model.ExitCodeConfigError, "validate config", fmt.Errorf("command %q tags: %w", id, err))
		}
		if err := validateParserLabel(cmd.Parser, true); err != nil {
			return err
		}
		if cmd.TimeoutSec <= 0 || cmd.TimeoutSec > 86400 {
			return model.NewGaoriError(model.ExitCodeConfigError, "validate config", fmt.Errorf("command %q timeout_sec must be between 1 and 86400", id))
		}
	}
	for _, pattern := range cfg.Redaction.Patterns {
		if strings.TrimSpace(pattern.Name) == "" {
			return model.NewGaoriError(model.ExitCodeConfigError, "validate config", fmt.Errorf("redaction pattern name must not be empty"))
		}
		if _, err := regexp.Compile(pattern.Regex); err != nil {
			return model.NewGaoriError(model.ExitCodeConfigError, "validate config", fmt.Errorf("redaction pattern %q invalid regex: %w", pattern.Name, err))
		}
	}
	return nil
}
...
func ValidateTags(values []string, operation string) ([]string, error) {
	...
	if err != nil {
		return nil, model.NewGaoriError(model.ExitCodeConfigError, operation, err)
	}
