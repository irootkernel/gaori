...
func Main(args []string, stdout, stderr io.Writer) int {
	return Run(args, stdout, stderr, NewBuildInfo("gaori", "0.1.11", "unknown", "unknown"))
}
...
func Run(args []string, stdout, stderr io.Writer, info BuildInfo) int {
	...
	if len(remaining) == 0 {
		writeLine(stderr, "usage: gaori [global options] <version|run|excerpt|summarize|clean|config|rules>")
		return int(model.ExitCodeConfigError)
	}
