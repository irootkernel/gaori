...
Guidance for coding agents working in `gaori` local development.

Gaori is a standalone deterministic Go CLI for running test commands, preserving raw logs, extracting bounded failure evidence, and writing compact summary/status artifacts.
...
- `docs/requirements-specs.md` for Gaori requirements and non-goals.
...
- Gaori remains standalone and deterministic unless an approved task changes that contract.
...
- Push back on requests that make Gaori a planner, reviewer, acceptance gate, waiver authority, or runtime orchestrator.
...
- `--run-id` artifacts must stay inside the matching `.gaori/runs/scoped/<run_id>/artifacts/test/` path and must not cross-run or symlink-escape.
- Standalone runs write under `.gaori/runs/standalone/<UTC-timestamp>[-NNN]/`.
...
```bash
HOME=/Users/draccoon gaori run unit
HOME=/Users/draccoon gaori run integration
HOME=/Users/draccoon gaori run e2e
HOME=/Users/draccoon go test ./...
git diff --check
```

Use `HOME=/Users/draccoon gaori run all` or `HOME=/Users/draccoon make test` when full local release-style verification is needed. The full `make test` path includes format/lint/vet/guardrails/unit/integration/e2e and may fail if optional lint tooling is unavailable.

Verification expectations:

- Parser/rule changes: focused parser/rule tests plus a Gaori run or summarize smoke.
